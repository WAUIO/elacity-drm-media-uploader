			...
			hooks: {
				after: {
					storeLocalFile: [
						...
						async function dashVideo(ctx: Context<any, {targetFlow?: string[]; $multipart?: IUploadMetadata}>, file: IFile) {
							if (ctx.meta.targetFlow?.includes("dash")) {
								...
								await ctx.call("bento.mp4fragment", {args: [
									[file.path, fragmentedFile, "--fragment-duration", "2000"],
								]});

								const outputDir = path.join(path.dirname(file.path), "output");
								await ctx.call("bento.mp4dash", {args: [
									fragmentedFile,
									[
						...
						// eslint-disable-next-line prefer-arrow/prefer-arrow-functions
						async function ipfs(ctx: Context<any, {targetFlow?: string[]; $multipart?: IUploadMetadata}>, file: IFile) {
